    ...
    <section id={service.slug} data-testid={`service-section-${service.slug}`} className="scroll-mt-24 border-t border-line">
      <div className={cn("container-x service-section-inner", flipped && "is-flipped")}>
        ...
        <div className="service-copy">
          ...
          <Reveal delay={.25} className="mt-9 flex flex-wrap items-center gap-x-7 gap-y-4 border-t border-line pt-6"><Link to={`/contact?type=${encodeURIComponent(service.projectType)}`} data-testid={`service-cta-${service.slug}`} className="btn-brand">Discuss this service<ArrowRight className="h-4 w-4" /></Link>{site.whatsapp.enabled && <a href={waLink(`Hi Suvi Interior, I'm interested in ${service.title.toLowerCase()} for my home and would like to discuss.`)} target="_blank" rel="noopener noreferrer" data-testid={`service-whatsapp-${service.slug}`} className="arrow-link text-charcoal"><WhatsAppIcon className="h-3.5 w-3.5" />WhatsApp</a>}</Reveal>
        </div>
      </div>
    </section>
  );
